...
import com.mycompany.cyberread.Service.PostService;
...
public class PostController {
    ...
    /**
     * Retrieves representation of an instance of com.mycompany.cyberread.Controller.PostController
    ...
    public Response getPostsByFollowedUsers(@HeaderParam("Token") String jwt) throws PostException {
        if(jwt == null) {
            ...
            switch(tokenCheckResult) {
                case 1:
                    ...
                    JSONArray result = PostService.getPostsByFollowedUsers(userId);
    ...
    @Path("addPost")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response addPost(@HeaderParam("Token") String jwt, Post p) throws PostException {
        if(jwt == null) {
            return  Response.status(Response.Status.UNAUTHORIZED).entity("User hasn't token!").build();
        } else {
            int tokenCheckResult = Token.decodeJwt(jwt);
            ...
            switch(tokenCheckResult) {
                case 1:
                    Integer userId = Token.getUserIdByToken(jwt);
                    String result = PostService.addPost(userId, p.getText());
